...
import { sendCustomerToCRM, syncCustomerWithCRM } from './crmService';
...
// Generate time slots for the entire day in 1-hour increments (midnight to midnight)
export const generateTimeSlots = (): string[] => {
  const slots: string[] = [];
  const startHour = 0; // 12 AM (midnight)
  const endHour = 23; // 11 PM (last slot starts at 11 PM)
...
export const createBooking = async (bookingData: {
  ...
  try {
    ...
    const bookingInsertData: any = {
      ...
      business_id: '6c5d19b9-e75c-48e6-a894-ca33882a0304', // Bond Chauffeur business ID
    ...
    // Only add new fields if they have values (to avoid database errors if columns don't exist yet)
    if (bookingData.pickupLocation) bookingInsertData.pickup_location = bookingData.pickupLocation;
    if (bookingData.dropoffLocation) bookingInsertData.dropoff_location = bookingData.dropoffLocation;
    if (bookingData.luggageCount) bookingInsertData.luggage_count = bookingData.luggageCount;
    if (bookingData.returnTrip !== undefined) bookingInsertData.return_trip = bookingData.returnTrip;
    if (bookingData.vehicleType) bookingInsertData.preferred_vehicle = bookingData.vehicleType;
    if (bookingData.flightNumber) bookingInsertData.flight_number = bookingData.flightNumber;
    ...
    // Send customer data to CRM (don't wait for this to complete)
    try {
      ...
      const crmBusinessId = '6c5d19b9-e75c-48e6-a894-ca33882a0304'; // This is the CRM business ID
      ...
      const newCrmResult = await syncCustomerWithCRM({
        ...
        restaurantName: 'Bond Chauffeur' // Always include the restaurant name
      });
      
      if (newCrmResult.success) {
        console.log('✅ Customer data synced with new CRM API successfully');
      } else {
        console.warn('⚠️ Failed to sync customer data with new CRM API:', newCrmResult.error);
      }
      
      // For backward compatibility, also send to the old CRM system
      const oldCrmResult = await sendCustomerToCRM({
        customerName: bookingData.customerName,
        customerEmail: bookingData.customerEmail,
        customerPhone: bookingData.customerPhone,
        specialRequests: bookingData.specialRequests,
      ...
      if (oldCrmResult.success) {
        console.log('✅ Customer data sent to old CRM successfully');
      } else {
        console.warn('⚠️ Failed to send customer data to old CRM:', oldCrmResult.error);
      }
    } catch (crmError) {
      console.warn('⚠️ Error sending customer data to CRM systems:', crmError);
...
export const fetchAllBookings = async (): Promise<Booking[]> => {
  console.log('Fetching all bookings from Supabase for business ID: 6c5d19b9-e75c-48e6-a894-ca33882a0304');
  const { data, error } = await supabase
    .from('bookings')
    .select(`*, restaurant:restaurant_id (id, name, cuisine)`)
    .eq('business_id', '6c5d19b9-e75c-48e6-a894-ca33882a0304')
  ...
  console.log('All bookings fetched for business ID 6c5d19b9-e75c-48e6-a894-ca33882a0304:', data);
  // Cast the data to ensure proper typing for status field
  return (data || []).map(booking => ({
    ...booking,
    status: booking.status as 'confirmed' | 'cancelled' | 'completed',
