...
// Get all bookings for Bond Chauffeur (for customer analysis)
export const getBondChauffeurBookings = async (): Promise<any[]> => {
  try {
    const { data, error } = await supabase
      .from('bookings')
      .select('*')
      .eq('business_id', '6c5d19b9-e75c-48e6-a894-ca33882a0304'); // Bond Chauffeur business ID

    if (error) throw error;
    return data || [];
  } catch (error) {
    console.error('Error fetching Bond Chauffeur bookings:', error);
    return [];
  }
};
...
// Get unique customers for Bond Chauffeur
export const getBondChauffeurCustomers = async (): Promise<any[]> => {
  try {
    const { data, error } = await supabase
      .from('customers')
      .select('*')
    ...
    console.error('Error fetching Bond Chauffeur customers:', error);
    return [];
  }
}; 